...
def GG_orange_white_colormap(N_levels=90):
    """
...
for i, product_ID in zip(range(len(product_IDs)), product_IDs):
    ...
    for j, feedstock_ID in zip(range(len(feedstock_IDs)), feedstock_IDs):
        ...
        dcon_array = results_metric_1 = np.load(f'{filename}_dcon.npy')
        ...
        top_titer_dcon_array = dcon_array[:, -1, :][0]
        has_infeas_region = True
        try:
            infeas_ur_yield_index = np.where(np.isnan(top_titer_dcon_array))[0][-1] + 1
        ...
        dcon_w_label =  r"$\bfContribution$"  + " " r"$\bfof$" + " "  r"$\bfd$" + " " + r"$\bfTerm$" + " " + r"$\bfto$" + " " + r"$\bfMPSP$"
        dcon_units = r"$\mathrm{\%}$"
        ...
        dcon_w_levels = np.arange(0, 1.01, 0.025)
        dcon_cbar_ticks = np.arange(0, 1.01, 0.1)
        # dcon_w_ticks = [1., 1.25, 1.5, 1.75, 2., 2.5,  3., 4, 5, 6, 8,]
        dcon_w_ticks = [0, 0.025, 0.05, 0.1, 0.15, 0.2, 0.4, 0.6, 0.8, 1.]
        
        # dcon_w_ticks = get_w_ticks(dcon_array, dcon_w_levels, n_ticks=3)
        # dcon_w_ticks = get_w_ticks_from_percentiles(dcon_array, dcon_w_levels, 
        #                                             percentiles=(25, 50, 75),
        #                                             # cbar_ticks=dcon_cbar_ticks,
        #                                             cbar_ticks=None,
        #                                             )
        
        contourplots.animated_contourplot(w_data_vs_x_y_at_multiple_z=dcon_array, # shape = z * x * y # values of the metric you want to plot on the color axis; e.g., dcon
                                        ...
                                        w_label=dcon_w_label, # title of the color axis
                                        x_ticks=x_ticks,
                                        y_ticks=y_ticks,
                                        z_ticks=z_ticks,
                                        w_levels=dcon_w_levels, # levels for unlabeled, filled contour areas (labeled and ticked only on color bar)
                                        w_ticks=dcon_w_ticks, # labeled, lined contours; a subset of w_levels
                                        x_units=x_units,
                                        y_units=y_units,
                                        z_units=z_units,
                                        w_units=dcon_units,
                                        ...
                                        cmap=GG_orange_white_colormap(), # can use 'viridis' or other default matplotlib colormaps
                                        ...
                                        animated_contourplot_filename=filename+'_dcon_y_t_sims', # file name to save animated contourplot as (no extensions)
...
plt.savefig(f'TAL_all_dcon_TRY_fit.png', 
            transparent = False,  
            facecolor = 'white',
            bbox_inches='tight',
            dpi=300,
...
contourplots.animated_contourplot(w_data_vs_x_y_at_multiple_z=dcon_array, # shape = z * x * y # values of the metric you want to plot on the color axis; e.g., dcon
                                ...
                                fps=fps, # animation frames (z values traversed) per second
                                n_loops='inf', # the number of times the animated contourplot should loop animation over z; infinite by default
                                animated_contourplot_filename='colorbar_dcon_TRY', # file name to save animated contourplot as (no extensions)
                                keep_frames=keep_frames, # leaves frame PNG files undeleted after running; False by default
                                axis_title_fonts=axis_title_fonts,
                                clabel_fontsize = clabel_fontsize,
                                default_fontsize = default_fontsize,
