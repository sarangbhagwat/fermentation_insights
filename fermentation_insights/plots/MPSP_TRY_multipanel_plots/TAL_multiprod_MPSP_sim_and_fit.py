...
for i, p in zip(range(len(productivities)), productivities):
# for i, product_ID in zip(range(len(product_IDs)), product_IDs):
    ...
    for j, column in zip(range(len(columns)), columns):
        ...
        if len(columns)>1:
            ...
            ax = axs_list
        p_over_baseline = p/0.12
        filename = product+f'_{round_to(p_over_baseline, 1)}bp_'+feedstock if not p_over_baseline==1 else product+'_'+feedstock
        print(filename)
...
plt.savefig(f'TAL_SA_sc_multiprod_TRY.png', 
            transparent = False,  
            facecolor = 'white',
            bbox_inches='tight',
            dpi=300,
