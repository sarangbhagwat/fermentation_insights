...
f = a + b*100/y +c/t + d*100/(y*t) # if yield increment to test is 1% theoretical rather than 100% theoretical, multiple b and d by 100
...
if plot_MPSP_y_t:
    ...
    contourplots.animated_contourplot(w_data_vs_x_y_at_multiple_z=indicator_array_for_plot, # shape = z * x * y # values of the metric you want to plot on the color axis; e.g., MPSP
                                    ...
                                    include_x_axis_ticklabels = False,
                                    include_y_axis_ticklabels = False,
                                    figwidth=None,
                                    fig_ax_to_use=(fig, axs[0]),
                                    )

    plt.subplots_adjust(wspace=0, hspace=0)
    fig.set_figwidth(9)
    
    plt.savefig(f'TAL_all_TRY.png', 
                transparent = False,  
                facecolor = 'white',
                bbox_inches='tight',
                dpi=300,
    ...
    ys_plot = 100*yields[:yield_upper_bound_index_for_eval]
    ts_plot = titers

    rel_impact_arr = np.array([[rel_impact_f(yi, ti) for yi in ys_plot] for ti in ts_plot])
