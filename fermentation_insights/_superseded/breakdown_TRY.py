...
#%% Sample separately for each yield regime
import chaospy

indices = []
ys, ts, mpsps = [], [], []
low_bound_index_yield, low_bound_index_titer = 0, 0
sampling_type = 'latin_hypercube'
for i in indices_inflection_product_yields:
    ...
    if sampling_type=='random':
        ...
        ts.append([])
        mpsps.append([])
        while len(indices[-1])<4:
            ind = [np.random.randint(low_bound_index_yield, i), np.random.randint(low_bound_index_titer, len(titers)-1)]
            if not np.any(np.isnan(indicator_array[0][ind[1], ind[0]])):
                indices[-1].append(ind)
                ys[-1].append(yields[ind[0]])
                ts[-1].append(titers[ind[1]])
                mpsps[-1].append(indicator_array[0][ind[1], ind[0]])
    
    elif sampling_type=='latin_hypercube':
        indic = np.array(list(zip(
                        np.int64(np.round(chaospy.Uniform(low_bound_index_yield, i).sample(4, rule='latin_hypercube', seed=3221),0)),
                        np.int64(np.round(chaospy.Uniform(low_bound_index_titer, len(titers)-1).sample(4, rule='random', seed=3221),0))
                      )))
...
ys, ts, mpsps, indices = np.array(ys), np.array(ts), np.array(mpsps), np.array(indices)

#%% Solve
# need to constrain all params > 0

# #%% Solve using fsolve
# from scipy.optimize import fsolve

# def equations(p, ydata, tdata, mpspdata):
#     a, b, c, d = p
#     return (a + b/ydata[0] + c/tdata[0] + d/(ydata[0]*tdata[0]) - mpspdata[0],
...
#     def eqns(p):
#         return equations(p, ydata, tdata, mpspdata)
#     a, b, c, d =  fsolve(eqns, (1, 1, 1, 1), xtol=1e-30, maxfev=100000)
#     print(eqns((a, b, c, d)))
#     avals.append(a)
#     bvals.append(b)
#     cvals.append(c)
...
# #%% Solve analytically and manually
# avals, bvals, cvals, dvals = [], [], [], []
# for ydata, tdata, mpspdata in zip(ys, ts, mpsps):
#     a + b/ydata[0] + c/tdata[0] + d/(ydata[0]*tdata[0]) - mpspdata[0]
#     a = mpspdata[0] - b/ydata[0] - c/tdata[0] - d/(ydata[0]*tdata[0])
    
#     b = mpspdata[1] - a - c/tdata[1] - d/(ydata[1]*tdata[1])
#     b = mpspdata[1] - (mpspdata[0] - b/ydata[0] - c/tdata[0] - d/(ydata[0]*tdata[0])) - c/tdata[1] - d/(ydata[1]*tdata[1])

#     c = mpspdata[2] - a - b/ydata[2] - d/(ydata[2]*tdata[2])
#     c = mpspdata[2] - a - b/ydata[2] - d/(ydata[2]*tdata[2])
...
for ydata, tdata, mpspdata in zip(ys, ts, mpsps):
    ...
    print(get_Rsq(mpspdata, np.array([shifted_rect_hyperbola_two_param(yt[0], yt[1], a, b, c, d) for yt in zip(ydata, tdata)])))
    print('\n')
    avals.append(a)
    bvals.append(b)
    cvals.append(c)
...
print(get_Rsq(indicator_array, np.array([[[MPSP_f(y, t, avals, bvals, cvals, dvals) for y in yields] for t in titers]])))
