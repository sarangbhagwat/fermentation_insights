...
from fermentation_insights.utils import loop_evaluate_across_param,\
        ...
        shifted_rect_hyperbola_two_param_6_coeff, fit_shifted_rect_hyperbola_two_param_6_coeff,\
        plot_across_param
...
#%%
def MPSP_f(y, t, avals, bvals, cvals, dvals, evals, fvals): # use the coefficients solved for the corresponding yield regime
    for ii, i, iy in zip(range(len(indices_inflection_product_yields)),
                         indices_inflection_product_yields, 
                         inflection_product_yields):
        if y<=iy:
            return shifted_rect_hyperbola_two_param_6_coeff(y, t, avals[ii], bvals[ii], cvals[ii], dvals[ii], evals[ii], fvals[ii])
    # return shifted_rect_hyperbola_two_param_6_coeff(y, t, avals[-1], bvals[-1], cvals[-1], dvals[-1])
...
def solve_for_coeffs(ys, ts, mpsps, yield_upper_bound_index_for_eval, print_results=True):
    avals, bvals, cvals, dvals, evals, fvals = [], [], [], [], [], []
    ...
    for ydata, tdata, mpspdata in zip(ys, ts, mpsps):
        ...
        cons = (
                ...
                {'type': 'eq', 'fun': lambda x: x[0] + x[1]/ydata[0] + x[2]/tdata[0] + x[3]/(ydata[0]*tdata[0]) + x[4]*ydata[0] + x[5]*tdata[0] - mpspdata[0]},
                {'type': 'eq', 'fun': lambda x: x[0] + x[1]/ydata[1] + x[2]/tdata[1] + x[3]/(ydata[1]*tdata[1]) + x[4]*ydata[1] + x[5]*tdata[1] - mpspdata[1]},
                {'type': 'eq', 'fun': lambda x: x[0] + x[1]/ydata[2] + x[2]/tdata[2] + x[3]/(ydata[2]*tdata[2]) + x[4]*ydata[2] + x[5]*tdata[2] - mpspdata[2]},
                {'type': 'eq', 'fun': lambda x: x[0] + x[1]/ydata[3] + x[2]/tdata[3] + x[3]/(ydata[3]*tdata[3]) + x[4]*ydata[3] + x[5]*tdata[3] - mpspdata[3]},
                {'type': 'eq', 'fun': lambda x: x[0] + x[1]/ydata[4] + x[2]/tdata[4] + x[3]/(ydata[4]*tdata[4]) + x[4]*ydata[4] + x[5]*tdata[4] - mpspdata[4]},
                {'type': 'eq', 'fun': lambda x: x[0] + x[1]/ydata[5] + x[2]/tdata[5] + x[3]/(ydata[5]*tdata[5]) + x[4]*ydata[5] + x[5]*tdata[5] - mpspdata[5]},
                
                )
        res = minimize(minimize_obj_f, (1, 1, 1, 1, 1, 1), 
                       method='SLSQP', 
                        # bounds=(
                        #     (0, None), # a
                        #     (0, None), # b
        ...
        if print_results:
            print(a, b, c, d, e, f)
            print(minimize_obj_f((a, b, c, d, e, f)))
            print(mpspdata)
            print(np.array([shifted_rect_hyperbola_two_param_6_coeff(yt[0], yt[1], a, b, c, d, e, f) for yt in zip(ydata, tdata)]))
            print('\n')
        try:
            rsq_check = get_Rsq(mpspdata, np.array([shifted_rect_hyperbola_two_param_6_coeff(yt[0], yt[1], a, b, c, d, e, f) for yt in zip(ydata, tdata)]))
            if print_results: print(rsq_check)
        except Exception as exc:
            print(f'Could not compute R^2 value: {str(exc)}.')
        avals.append(a)
        bvals.append(b)
        cvals.append(c)
        dvals.append(d)
    ...
    Rsq = (get_Rsq(indicator_array[:, :, :yield_upper_bound_index_for_eval], np.array([[[MPSP_f(y, t, avals, bvals, cvals, dvals, evals, fvals) for y in yields[:yield_upper_bound_index_for_eval]] for t in titers]])))

    return Rsq, res.success, avals, bvals, cvals, dvals, evals, fvals

Rsq, success, avals, bvals, cvals, dvals, evals, fvals = solve_for_coeffs(ys=ys, ts=ts, mpsps=mpsps, yield_upper_bound_index_for_eval=yield_upper_bound_index_for_eval)
...
a_fit, b_fit, c_fit, d_fit, r = fit_shifted_rect_hyperbola_two_param_6_coeff((ytarr[0], ytarr[1]), 
                                                                           np.array(indicator_array)[0, :, :yield_upper_bound_index_for_eval].flatten()[non_nan_indices],
                                                                           [0,0,0,0,])
...
print(get_Rsq(indicator_array[:, :, :yield_upper_bound_index_for_eval], np.array([[[MPSP_f(y, t, avals, bvals, cvals, dvals, evals, fvals) for y in yields[:yield_upper_bound_index_for_eval]] for t in titers]])))
...
def sample_and_solve(yields, 
    ...
    # print(indices)
    if len(ys[0])<6: return None # with non-repeating indices rule, there are not enough valid indices in the current sampling region
    Rsq, success, avals, bvals, cvals, dvals, evals, fvals = solve_for_coeffs(ys=ys, ts=ts, mpsps=mpsps, yield_upper_bound_index_for_eval=yield_upper_bound_index_for_eval,
                                                       print_results=False)
    # print(Rsq, avals, bvals, cvals, dvals)
    return Rsq, ys[0], ts[0], success, avals, bvals, cvals, dvals, evals, fvals
...
all_yt_samples = []
unc_Rsqs = []
unc_as, unc_bs, unc_cs, unc_ds, unc_es, unc_fs = [], [], [], [], [], []
n_solves = 10000
i=0
fail_ct = 0
while len(unc_Rsqs)<n_solves:
    Rsq, ys, ts, success, avals, bvals, cvals, dvals, evals, fvals = sample_and_solve(yields=yields, 
                    titers=titers, 
    ...
    unc_es.append(evals[0])
    unc_fs.append(fvals[0])
    if not success: 
        fail_ct +=1
        print(success)
    i+=1
...
for region_to_sample in regions_to_sample:
    ...
    while i<(region_to_sample+1)*solves_stepsize:
        ...
        result = sample_and_solve(yields=yields, 
                        ...
                        n_samples=6, 
                        seed=None,
                        ind_ind_for_MPSP_y=ind_ind_for_MPSP_t, 
                        ind_ind_for_MPSP_t=ind_ind_for_MPSP_t, 
                        ...
                        yield_upper_bound_index_for_eval=indices_inflection_product_yields[0]+1,
                        repeating_indices_allowed=True,
                        repeating_points_allowed=True,
                        )
        # print(result)
        if not result is None:
            # print(1)
            Rsq, ys, ts, success, avals, bvals, cvals, dvals, evals, fvals = result
            # print(ys, ts)
            # print('\n')
