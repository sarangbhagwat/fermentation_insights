...
#%% HP TRY
from biorefineries.HP.analyses.fermentation.TRY_analysis_glucose_Acrylic import results_metric_1, yields, titers_mol_per_mol_total, productivities,\
                                                            colors, CABBI_green_colormap, get_rounded_str,\
                                                            AA_market_range, titers as titers_gpL

# titers = np.array(titers_mol_per_mol_total[5])
titers = titers_gpL * 1/(90.08*55.55)
...
for i in range(len(results_metric_1)):
    ...
    for j in range(len(results_metric_1[0,0,:])):
        ...
        MPSPs = results_metric_1[0,:,j][:to_index]
        titers_curr = titers[:to_index]
        
        
        a, b, substrate_sugars_mol, r = fit_min_sep_energy_based_f(tdata=titers_curr, indicatordata=MPSPs, y=y, p0=[10, 10, 400])
        az.append(a)
        bs.append(b)
        substrate_sugars_mols.append(substrate_sugars_mol)
        plt.scatter(titers_curr, MPSPs)
        plt.plot(titers_curr, [min_sep_energy_based_f(t, a, b, substrate_sugars_mol, y) for t in titers_curr])
        plt.show()
    ...
    if plot_coefficients_vs_yield:
        ...
        ax.scatter(yields, az, label='orig') # from MPSP*titer vs titer straight line fit
        # ax.plot(yields, az,
        ...
        ax.set_ylabel(f'b')
        ax.set_xlabel(f'{product_ID} yield[% theoretical]')
        ax.scatter(yields, bs, label='orig') # from MPSP*titer vs titer straight line fit
        # ax.plot(yields, bs,
        #         label='fit') # from (orig)*yield vs yield piecewise linear fit
        
        ax = plt.subplot(313)
        
        ax.set_ylabel(f'substrate_sugars_mol')
        ax.set_xlabel(f'{product_ID} yield[% theoretical]')
        ax.scatter(yields, substrate_sugars_mols, label='orig') # from MPSP*titer vs titer straight line fit
        # ax.plot(yields, substrate_sugars_mols,
        #         label='fit') # from (orig)*yield vs yield piecewise linear fit
...
def MPSP_y_t_f(t, az, bs, substrate_sugars_mols, y):
    yield_index = np.where(yields==y)[0][0]
    a = az[yield_index]
    b = bs[yield_index]
    substrate_sugars_mol = substrate_sugars_mols[yield_index]
    return min_sep_energy_based_f(t, a, b, substrate_sugars_mol, y)
...
if plot_MPSP_y_t:
    ...
    contourplots.animated_contourplot(w_data_vs_x_y_at_multiple_z=results_metric_1, # shape = z * x * y # values of the metric you want to plot on the color axis; e.g., MPSP
                                    ...
                                    y_data=titers_gpL, # y axis values
                                    ...
                                    fps=2, # animation frames (z values traversed) per second
    ...
    contourplots.animated_contourplot(w_data_vs_x_y_at_multiple_z=[[[MPSP_y_t_f(t, az, bs, substrate_sugars_mols, y) for y in yields] for t in titers]], # shape = z * x * y # values of the metric you want to plot on the color axis; e.g., MPSP
                                    x_data=100*yields, # x axis values
                                    # x_data = yields/theoretical_max_g_TAL_acid_per_g_glucose,
                                    y_data=titers_gpL, # y axis values
                                    z_data=productivities, # z axis values
