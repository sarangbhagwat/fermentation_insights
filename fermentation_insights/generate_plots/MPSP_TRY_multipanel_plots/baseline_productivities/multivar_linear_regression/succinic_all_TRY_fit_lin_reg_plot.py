...
for i, product_ID in zip(range(len(product_IDs)), product_IDs):
    ...
    for j, feedstock_ID in zip(range(len(feedstock_IDs)), feedstock_IDs):
        ...
        print(filename)
        yields = yields_for_plot = np.load(filename+'_yields_for_eval_lin_reg.npy')
        titers = titers_for_plot = np.load(filename+'_titers_for_eval_lin_reg.npy')
        indicator_array = results_metric_1 = np.load(f'{filename}_MPSP_fit_lin_reg.npy')
        ...
        coefficients = np.load(f'{filename}_coefficients_lin_reg.npy')
        Rsq = coefficients[-1]
...
plt.savefig(f'succinic_all_TRY_fit_lin_reg.png', 
            transparent = False,  
            facecolor = 'white',
            bbox_inches='tight',
            dpi=300,
