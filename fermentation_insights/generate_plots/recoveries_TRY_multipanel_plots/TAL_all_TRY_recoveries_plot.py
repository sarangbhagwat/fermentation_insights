...
for i, product_ID in zip(range(len(product_IDs)), product_IDs):
    ...
    for j, feedstock_ID in zip(range(len(feedstock_IDs)), feedstock_IDs):
        ...
        top_titer_recoveries_array = recoveries_array[:, -1, :][0]
        has_infeas_region = True
        try:
            infeas_ur_yield_index = np.where(np.isnan(top_titer_recoveries_array))[0][-1] + 1
        ...
        recovery_units = r"$\mathrm{kg-p}\cdot\mathrm{kg-fp}$"
        ...
        recovery_w_levels = np.arange(0., 1.51, 0.05)
        recovery_cbar_ticks = np.arange(0., 1.51, 0.1)
        # recovery_w_ticks = [1., 1.25, 1.5, 1.75, 2., 2.5,  3., 4, 5, 6, 8,]
        # recovery_w_ticks = [3, 5, 8]
        # recovery_w_ticks = get_w_ticks(recoveries_array, recovery_w_levels, n_ticks=3)
        ...
        contourplots.animated_contourplot(w_data_vs_x_y_at_multiple_z=recoveries_array, # shape = z * x * y # values of the metric you want to plot on the color axis; e.g., MPSP
                                        ...
                                        animated_contourplot_filename=filename+'_recovery_y_t_sims', # file name to save animated contourplot as (no extensions)
...
plt.savefig(f'TAL_all_TRY_recoveries.png', 
            transparent = False,  
            facecolor = 'white',
            bbox_inches='tight',
            dpi=300,
...
recovery_w_levels = np.arange(0., 1.51, 0.05)
recovery_cbar_ticks = np.arange(0., 1.51, 0.25)
recovery_w_ticks = []

contourplots.animated_contourplot(w_data_vs_x_y_at_multiple_z=recoveries_array, # shape = z * x * y # values of the metric you want to plot on the color axis; e.g., recovery_
                                x_data=yields_for_plot, # x axis values
                                # x_data = yields/theoretical_max_g_HP_acid_per_g_glucose,
                                y_data=titers_for_plot, # y axis values
                                z_data=np.array([1]), # z axis values
                                ...
                                fps=fps, # animation frames (z values traversed) per second
                                n_loops='inf', # the number of times the animated contourplot should loop animation over z; infinite by default
                                animated_contourplot_filename='colorbar_recovery_TRY', # file name to save animated contourplot as (no extensions)
                                keep_frames=keep_frames, # leaves frame PNG files undeleted after running; False by default
                                axis_title_fonts=axis_title_fonts,
                                clabel_fontsize = clabel_fontsize,
                                default_fontsize = default_fontsize,
