...
for i, product_ID in zip(range(len(product_IDs)), product_IDs):
    ...
    for j, feedstock_ID in zip(range(len(feedstock_IDs)), feedstock_IDs):
        ...
        resMPSPfrac_array = results_metric_1 = np.load(f'{filename}_resMPSPfrac.npy')
        ...
        top_titer_resMPSPfrac_array = resMPSPfrac_array[:, -1, :][0]
        has_infeas_region = True
        try:
            infeas_ur_yield_index = np.where(np.isnan(top_titer_resMPSPfrac_array))[0][-1] + 1
        ...
        resMPSPfrac_w_label =  r"$\bfContribution$"  + " " r"$\bfof$" + " "  r"$\bfd$" + " " + r"$\bfTerm$" + " " + r"$\bfto$" + " " + r"$\bfMPSP$"
        resMPSPfrac_units = r"$\mathrm{\%}$"
        ...
        # resMPSPfrac_w_ticks = get_w_ticks(resMPSPfrac_array, resMPSPfrac_w_levels, n_ticks=3)
        # resMPSPfrac_w_ticks = get_w_ticks_from_percentiles(resMPSPfrac_array, resMPSPfrac_w_levels, 
        #                                             percentiles=(25, 50, 75),
        #                                             # cbar_ticks=resMPSPfrac_cbar_ticks,
        #                                             cbar_ticks=None,
        #                                             )
        
        contourplots.animated_contourplot(w_data_vs_x_y_at_multiple_z=resMPSPfrac_array, # shape = z * x * y # values of the metric you want to plot on the color axis; e.g., resMPSPfrac
                                        ...
                                        w_label=resMPSPfrac_w_label, # title of the color axis
                                        x_ticks=x_ticks,
                                        y_ticks=y_ticks,
                                        z_ticks=z_ticks,
                                        w_levels=resMPSPfrac_w_levels, # levels for unlabeled, filled contour areas (labeled and ticked only on color bar)
                                        w_ticks=resMPSPfrac_w_ticks, # labeled, lined contours; a subset of w_levels
                                        x_units=x_units,
                                        y_units=y_units,
                                        z_units=z_units,
                                        ...
                                        animated_contourplot_filename=filename+'_resMPSPfrac_y_t_sims', # file name to save animated contourplot as (no extensions)
...
plt.savefig(f'TAL_all_resMPSPfrac_TRY_fit.png', 
            transparent = False,  
            facecolor = 'white',
            bbox_inches='tight',
            dpi=300,
...
resMPSPfrac_w_levels = np.arange(-0.15, 0.1501, 0.005)
resMPSPfrac_cbar_ticks = np.arange(-0.15, 0.1501, 0.03)
# resMPSPfrac_w_ticks = [0, 0.025, 0.05, 0.1, 0.15, 0.2, 0.4, 0.6, 0.8, 1.]
resMPSPfrac_w_ticks = []
...
contourplots.animated_contourplot(w_data_vs_x_y_at_multiple_z=resMPSPfrac_array, # shape = z * x * y # values of the metric you want to plot on the color axis; e.g., resMPSPfrac
                                ...
                                fps=fps, # animation frames (z values traversed) per second
                                n_loops='inf', # the number of times the animated contourplot should loop animation over z; infinite by default
                                animated_contourplot_filename='colorbar_resMPSPfrac_TRY', # file name to save animated contourplot as (no extensions)
                                keep_frames=keep_frames, # leaves frame PNG files undeleted after running; False by default
                                axis_title_fonts=axis_title_fonts,
                                clabel_fontsize = clabel_fontsize,
                                default_fontsize = default_fontsize,
