...
for i, product_ID in zip(range(len(product_IDs)), product_IDs):
    ...
    for j, feedstock_ID in zip(range(len(feedstock_IDs)), feedstock_IDs):
        ...
        resMPSPfrac_w_label = r"$\bfRelative$" + " " + r"$\bfImpact$"
        resMPSPfrac_units = r"$\mathrm{kg-sugars}\cdot\mathrm{L-broth}^{-1}$"
...
plt.savefig(f'succinic_all_resMPSPfrac_TRY_fit.png', 
            transparent = False,  
            facecolor = 'white',
            bbox_inches='tight',
            dpi=300,
