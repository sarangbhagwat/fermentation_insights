...
if product in ('succinic', 'succinic_neutral', 'succinic_hexanol', 'succinic_neutral_hexanol'): theo_max_yield = 1.
...
# from biorefineries.succinic.analyses.full.plot_utils import plot_kde_formatted
...
parameter_distributions_filename = succinic_filepath+'\\analyses\\parameter_distributions\\'+'parameter-distributions_' + mode + '.xlsx'
...
def reset_and_reload():
    print('Resetting cache and not emptying recycles ...')
    system.reset_cache()
...
for yt in yts:
    try:
        ...
        # load samples again for each uncertainty analysis
        model.load_samples(samples)
