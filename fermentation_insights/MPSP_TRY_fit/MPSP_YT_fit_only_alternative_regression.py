...
from fermentation_insights.utils import loop_evaluate_across_param,\
        ...
        alternative_eq, fit_alternative_eq,\
        plot_across_param
...
def get_all_MPSP_yt_fit_alt_reg():
    not_found = []
    for i in all_filenames:
        print(i[0], i[1])
        for additional_tag in ('', '0.2bp', '5.0bp'):
            try:
                get_MPSP_yt_fit_alt_reg(product=i[0], feedstock=i[1], additional_tag=additional_tag, plot_MPSP_y_t=False)
            except:
                not_found.append((i[1], i[0], additional_tag))
        

        if i[0]+'_'+i[1] in ['TAL_SA_sugarcane']:
            get_MPSP_yt_fit_alt_reg(product=i[0], feedstock=i[1], additional_tag='0.2bp', plot_MPSP_y_t=False)
            # get_MPSP_yt_fit_alt_reg(product=i[0], feedstock=i[1], additional_tag='1.0bp', plot_MPSP_y_t=False)
            get_MPSP_yt_fit_alt_reg(product=i[0], feedstock=i[1], additional_tag='1.8bp', plot_MPSP_y_t=False)
            get_MPSP_yt_fit_alt_reg(product=i[0], feedstock=i[1], additional_tag='2.6bp', plot_MPSP_y_t=False)
            get_MPSP_yt_fit_alt_reg(product=i[0], feedstock=i[1], additional_tag='3.4bp', plot_MPSP_y_t=False)
            get_MPSP_yt_fit_alt_reg(product=i[0], feedstock=i[1], additional_tag='4.2bp', plot_MPSP_y_t=False)
            get_MPSP_yt_fit_alt_reg(product=i[0], feedstock=i[1], additional_tag='5.0bp', plot_MPSP_y_t=False)
...
def get_MPSP_yt_fit_alt_reg(product, feedstock, additional_tag='', 
                    plot_MPSP_y_t=False, 
    ...
    np.save(f'{filename}_yields_for_eval_alt_reg.npy', yields_for_eval)
    np.save(f'{filename}_titers_for_eval_alt_reg.npy', titers_for_eval)
    np.save(f'{filename}_MPSP_for_eval_alt_reg.npy', indicator_array_for_eval)
    
    
    np.save(f'{filename}_recoveries_for_eval_alt_reg.npy', recoveries[:, :, :yield_upper_bound_index_for_eval])
    
    #%%
    def MPSP_f(y, t, a, b, c, d): # use the coefficients solved for the corresponding yield regime
    ...
    #%% Multivariate linear regression entire data
    ...
    # a_fit, b_fit, c_fit, d_fit, r = fit_alternative_eq((ytarr[0], ytarr[1]), 
    ...
    a_fit, b_fit, c_fit, d_fit, r = fit_alternative_eq((ys_fit, ts_fit), 
                                                        mpsps_for_fit,
                                                        [1,1,1,1],
                                                        # [0,0,0,0,0],
                                                        )
    ...
    np.save(
            filename+'_coefficients_alt_reg',
            np.array(
                    [a_fit, 
                     b_fit, 
                     c_fit, 
    ...
    np.save(f'{filename}_MPSP_fit_alt_reg.npy', fit_indicator_array_for_plot)
    ...
    np.save(f'{filename}_resMPSPfrac_alt_reg.npy', resMPSPfrac)
