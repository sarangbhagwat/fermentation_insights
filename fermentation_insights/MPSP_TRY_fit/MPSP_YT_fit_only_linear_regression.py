...
def get_MPSP_yt_fit_lin_reg(product, feedstock, additional_tag='', 
    ...
    np.save(f'{filename}_yields_for_eval_lin_reg.npy', yields_for_eval)
    np.save(f'{filename}_titers_for_eval_lin_reg.npy', titers_for_eval)
    np.save(f'{filename}_MPSP_for_eval_lin_reg.npy', indicator_array_for_eval)
    
    
    np.save(f'{filename}_recoveries_for_eval_lin_reg.npy', recoveries[:, :, :yield_upper_bound_index_for_eval])
    
    #%%
    def MPSP_f(y, t, a, b, c): # use the coefficients solved for the corresponding yield regime
        return a + b*y + c*t
    ...
    # a_fit, b_fit, c_fit, d_fit, r = fit_multivariate_linear_eq((ytarr[0], ytarr[1]), 
    ...
    a_fit, b_fit, c_fit, r = fit_multivariate_linear_eq((ys_fit, ts_fit), 
                                                        mpsps_for_fit,
                                                        [1,1,1],
                                                        # [0,0,0,0,0],
                                                        )
    ...
    print(get_Rsq(indicator_array_for_eval, 
                  np.array([[[MPSP_f(y, t, a_fit, b_fit, c_fit) 
    ...
    np.save(
            filename+'_coefficients_lin_reg',
            np.array(
                    [a_fit, 
                     b_fit, 
                     c_fit, 
    ...
    #%% MPSP_fit
    fit_indicator_array_for_plot = [[[MPSP_f(y, t, a_fit, b_fit, c_fit) 
    ...
    np.save(f'{filename}_resMPSPfrac_lin_reg.npy', resMPSPfrac)
